...
| TBD-09 | 画像 URL のキャッシュ効率 | 期限付きの署名付き URL を毎回発行しているため、同じ画像（プロフィール・投稿とも）でもブラウザキャッシュが効かず再取得になる。CloudFront + 署名付き Cookie / OAC に移行して URL を安定させるかは今後判断する（[F07_profile.md](features/F07_profile.md) 5.） | 未決 |
